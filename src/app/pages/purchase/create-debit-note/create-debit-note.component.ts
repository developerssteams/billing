...
export class CreateDebitNoteComponent implements OnInit {
  ...
  saveApiUrl = 'https://billsezy.com/Api/debit-notes.php';
  ...
  // ================= NAVIGATION =================
  goBack() {
    this.router.navigate(['/purchase/debit-notes']);
  }
  ...
  save() {
    ...
    // Get date values
    const dateInputs = document.querySelectorAll('.form-field input[type="date"]');
    this.supplierInvoiceDate = (dateInputs[0] as HTMLInputElement)?.value || '';
    this.paymentByDate = (dateInputs[1] as HTMLInputElement)?.value || '';

    // Set payable amount based on payment option
    if (this.paymentOption === 'full') {
      this.payableAmount = this.getFinalTotalWithRoundOff();
    }
    ...
    const payload = {
      ...
      item_discount_total: this.getTotalItemDiscount(),
    ...
    this.http.post(this.saveApiUrl, payload).subscribe({
      next: (response: any) => {
        ...
        // ✅ FIXED: Handle both types of success responses
        ...
        if (isSuccess) {
          ...
          setTimeout(() => {
            ...
            this.router.navigate(['/purchase/debit-notes']).then((success) => {
              console.log('Router navigation:', success);
              if (!success) {
                // Method 2: Window location fallback
                window.location.href = '/purchase/debit-notes';
              }
            }).catch(err => {
          ...
          alert('Error: ' + (response.message || 'Failed to create debit note'));
        }
      },
      error: (err) => {
  ...
  resetForm() {
    ...
    // Clear date inputs
    const dateInputs = document.querySelectorAll('.form-field input[type="date"]');
    if (dateInputs[0]) (dateInputs[0] as HTMLInputElement).value = '';
    if (dateInputs[1]) (dateInputs[1] as HTMLInputElement).value = '';
  }
}